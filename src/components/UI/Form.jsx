import React, { useEffect, useState } from "react";
import { Datepicker, Dropdown } from "flowbite-react";
import { getPopulationByBlockGroup } from "../../services/chicagoApi";
import classes from "../UI/UI.module.css";
import { useNavigate } from "react-router-dom";

export default function Form({ onGetCrimeData, isLoading }) {
	const navigate = useNavigate();
	const [selectedDate, setSelectedDate] = useState(null);

	const [address, setAddress] = useState({
		street: "",
		city: "Chicago",
		state: "Ilinois",
		zip: "",
	});

	const [radius, setRadius] = useState(1000);

	function handleSubmit(e) {
		e.preventDefault();
		console.log(radius, "radius");
		const formatted = new Date(selectedDate).toISOString().split("T")[0];
		("123 N Michigan Ave, Chicago, IL 60601");
		const formattedAddress = `${address.street}, ${address.city}, ${address.state} ${address.zip}`;
		onGetCrimeData({ address: formattedAddress, radius }, navigate);
	}
	function handleAddressChange(e) {
		const { value, id } = e.target;
		setAddress((prev) => ({ ...prev, [id]: value }));
	}
	const radiusList = [500, 1000, 1500];
	const today = new Date();
	const oneWeekBeforeToday = new Date(today);
	oneWeekBeforeToday.setDate(today.getDate() - 7);

	// useEffect(() => {
	// 	const blockGroup = "1";
	// 	const tract = "004200";
	// 	const county = "005"; // Change to a known county FIPS
	// 	const state = "36"; // New York

	// 	getPopulationByBlockGroup(blockGroup, tract, county, state)
	// 		.then((population) => {
	// 			console.log(`Population: ${population}`);
	// 		})
	// 		.catch((error) => {
	// 			console.error("Error:", error);
	// 		});
	// }, []);

	return (
		<>
			<form onSubmit={handleSubmit} className={classes.form}>
				<div className={classes.form_inputs}>
					<label htmlFor="address">Enter Street</label>
					<input
						className={classes.form_input_field}
						id="street"
						value={address.street}
						placeholder="123 sample st"
						required
						onChange={handleAddressChange}
					/>
				</div>
				<div className={classes.address}>
					<div className={classes.form_inputs}>
						<label htmlFor="address">City</label>
						<input
							className={classes.form_input_field}
							id="city"
							disabled
							value={address.city}
							placeholder="Chicago"
							required
							onChange={handleAddressChange}
						/>
					</div>
					<div className={classes.form_inputs}>
						<label htmlFor="address">State</label>
						<input
							className={classes.form_input_field}
							id="address"
							disabled
							value="Ilinois"
							required
							onChange={(e) => setAddress(e.target.value)}
						/>
					</div>
					<div className={classes.form_inputs}>
						<label htmlFor="address">Zip Code</label>
						<input
							className={classes.form_input_field}
							id="zip"
							value={address.zip}
							placeholder="xxxxx"
							required
							onChange={handleAddressChange}
						/>
					</div>
				</div>
				{/* <div className={classes.form_inputs}>
					<label htmlFor="radius">Select Radius:</label>
					<select
						className={classes.form_input_field}
						name="radius"
						id="radius"
						value={radius}
						onChange={(e) => setRadius(e.target.value)}
					>
						{radiusList.map((ele, index) => (
							<option key={index} value={ele}>
								{`${ele} mts`}
							</option>
						))}
					</select>
				</div> */}
				<div className={classes.form_btn}>
					<button
						disabled={isLoading}
						className="primary_btn flex justify-center"
					>
						{isLoading ? (
							<div role="status">
								<svg
									aria-hidden="true"
									className="w-8 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
									viewBox="0 0 100 101"
									fill="none"
									xmlns="http://www.w3.org/2000/svg"
								>
									<path
										d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
										fill="currentColor"
									/>
									<path
										d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
										fill="currentFill"
									/>
								</svg>
							</div>
						) : (
							"Find out"
						)}
					</button>
				</div>
			</form>
		</>
	);
}
